		...
		else
		{
			...
			if(g_ZMC_bIfDebugtoFile == 1)		//错误输出
			{
				if(ERR_OK != iresult) 
				{
					...
					str.Format("%s ", time_str); 
		
					str += pszCommand;

					str+="\tError:";
					CString str2;
					str2.Format("%d\t",iresult);
					str+=str2;

					str+="\r\n";
					fputs(str, DebugFileId);
				}
...
Return:         //错误码
*************************************************************/
int32 __stdcall ZAux_GetOutMulti(ZMC_HANDLE handle, uint16 iofirst,uint16 ioend, uint32 * istate)
{
	int32 iresult;
	if(NULL == istate || iofirst > ioend)
	{
